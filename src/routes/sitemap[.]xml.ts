...
import { GKE_ARTICLE_PATHS, SITE_ORIGIN } from '#/config/site'
  ...
  server: {
    handlers: {
      GET: () => {
        ...
        urls.push(
          localizeUrl(new URL(GKE_ARTICLE_PATHS.en, SITE_ORIGIN), {
            locale: 'en',
          }).href,
          localizeUrl(new URL(GKE_ARTICLE_PATHS.fr, SITE_ORIGIN), {
            locale: 'fr',
          }).href,
        )
